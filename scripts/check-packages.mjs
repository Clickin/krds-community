...
const checkPublishTarget = async (directory, target, label) => {
  ...
  if (Array.isArray(target)) {
    ...
    for (const [index, candidate] of target.entries()) {
      await checkPublishTarget(directory, candidate, `${label}[${index}]`);
    }
    return;
  }
  if (target && typeof target === 'object') {
    ...
    for (const [condition, candidate] of conditions) {
      await checkPublishTarget(directory, candidate, `${label}.${condition}`);
    }
    return;
  }
...
for (const entry of entries) {
  ...
  if (packageJson.license !== 'Apache-2.0') failures.push(`${entry}: Apache-2.0 license is required`);
  ...
  if (!packageJson.exports) {
    ...
    await checkPublishTarget(directory, packageJson.exports, `${entry}: exports`);
  }
