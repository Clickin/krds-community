...
const captureVisualSignatureAtCurrentOrigin = async (locator) =>
  locator.evaluate(async (root) => {
    ...
    const unmodeledPaintTags = new Set([
      'AUDIO',
      'CANVAS',
      'EMBED',
      'IFRAME',
    ...
    const imageValue = async (element) => {
      ...
      return {
        ...
        alt: element.alt,
        content:
          source && element.complete ? await hashAsset(source) : pixelFallback('image'),
      };
    };
    const serialize = async (node) => {
      ...
      const [
        ...
        getComputedStyle(node).display === 'list-item'
          ? pseudoValue(node, '::marker')
          : null,
...
const sameCoordinate = (left, right) =>
  Math.abs(left - right) <= visualCoordinateTolerance;
...
const withCanonicalVisualOrigin = async (locator, origin, operation) => {
  ...
  const transaction = await locator.evaluate((root, target) => {
    ...
    if (context && context !== root) {
      ...
      const numericStyle = (property) =>
        Number.parseFloat(contextStyle.getPropertyValue(property));
      ...
      if ([marginLeft, marginTop, paddingLeft, paddingTop].every(Number.isFinite)) {
        ...
        if (deltaY >= 0) {
          context.style.setProperty(
            'height',
            `${contextBounds.height + deltaY}px`,
            'important',
          );
