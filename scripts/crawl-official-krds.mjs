...
const SEED_CONFIGS = [
  { section: 'root', url: 'https://www.krds.go.kr/html/site/index.html', defaultFile: 'index.html' },
  { section: 'guide', url: 'https://www.krds.go.kr/html/site/style/style_01.html', defaultFile: 'style_01.html' },
  { section: 'components', url: 'https://www.krds.go.kr/html/site/component/component_summary.html', defaultFile: 'component_summary.html' },
  { section: 'service-patterns', url: 'https://www.krds.go.kr/html/site/service/service_summary.html', defaultFile: 'service_summary.html' },
  { section: 'basic-patterns', url: 'https://www.krds.go.kr/html/site/global/global_summary.html', defaultFile: 'global_summary.html' },
];
...
function isAssetUrl(urlString) {
  ...
  if (urlString.includes('google-analytics.com') || urlString.includes('googletagmanager.com')) return false;
  if (urlString.includes('github.com') || urlString.includes('naver.com') || urlString.includes('mois.go.kr')) {
    // Only capture static media/font/css/js from external domains
    return /\.(css|js|png|jpg|jpeg|gif|svg|webp|woff|woff2|ttf|eot|otf|pdf|mp4)$/i.test(urlString.split('?')[0]);
  }
  return true;
}
...
async function main() {
  ...
  page.on('response', async (response) => {
    try {
      ...
      if (pageMap.has(resUrl.split('#')[0].split('?')[0]) && parsedUrl.pathname.endsWith('.html')) return;
      ...
      const isAssetType =
        ...
        /\.(css|js|png|jpg|jpeg|gif|svg|webp|woff|woff2|ttf|eot|otf|pdf|mp4)$/i.test(parsedUrl.pathname);
  ...
  for (const [pageUrl, pageInfo] of pageMap.entries()) {
    ...
    doc.querySelectorAll('source, iframe, video, audio').forEach(el => rewriteAttributeUrl(el, 'src'));
  ...
  const manifestData = {
    ...
    sections: {
      ...
      'service-patterns': Array.from(pageMap.values()).filter(p => p.section === 'service-patterns').length,
      'basic-patterns': Array.from(pageMap.values()).filter(p => p.section === 'basic-patterns').length,
    },
  ...
  await writeFile(join(OUTPUT_DIR, 'manifest.json'), JSON.stringify(manifestData, null, 2), 'utf-8');
