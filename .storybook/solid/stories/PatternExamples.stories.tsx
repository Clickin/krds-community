...
const PatternPreview = (props: { patternId: string }): JSX.Element => {
  const [open, setOpen] = createSignal(true);
  const [checked, setChecked] = createSignal(false);
  const [rating, setRating] = createSignal("good");
  const [text, setText] = createSignal("");
  const [notifications, setNotifications] = createSignal(true);
  const form = (children: JSX.Element) => (
    <form class="krds-form-group" onSubmit={(event) => event.preventDefault()}>
      {children}
    </form>
  );
  ...
  if (props.patternId === "application")
    return form(
      <>
  ...
  if (props.patternId === "personal-information")
    return form(
      <>
  ...
  if (props.patternId === "list")
    return (
      <>
        ...
        <Pagination current={1} items={[1, 2, 3, 4, 5]} />
      </>
    );
  if (props.patternId === "feedback")
    return form(
      <>
        <fieldset>
  ...
  if (props.patternId === "detail")
    return (
      <>
        <span class="krds-badge">접수 완료</span>
        <h2>민원 신청 상세</h2>
        <dl>
          <dt>신청 번호</dt>
          <dd>2026-00124</dd>
  ...
  if (props.patternId === "filter-sort")
    return form(
      <>
        <fieldset>
          <legend>처리 상태</legend>
  ...
  if (props.patternId === "confirm")
    return (
      <>
        <h2>신청 내용 확인</h2>
        <p>제출 전에 입력한 내용을 확인하세요.</p>
        <Button onClick={() => setOpen(true)}>확인 열기</Button>
        <Modal title="신청서를 제출할까요?" open={open()} confirmLabel="제출" />
      </>
    );
  if (props.patternId === "mobile-notification")
    return (
      <>
        <h2>알림</h2>
        <span class="krds-badge">새 알림</span>
...
const meta = { title: "SolidJS/Patterns", parameters: { layout: "padded" } } satisfies Meta;
export default meta;
type Story = StoryObj<typeof meta>;
const story = (patternId: string): Story => ({
  render: () => {
    const root = document.createElement("main");
    root.style.cssText = "max-width:45rem;margin:0 auto";
    render(() => <PatternPreview patternId={patternId} />, root);
    return root;
  },
});
