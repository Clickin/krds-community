...
export const baseProps = (definition: FixtureDefinition): Record<string, unknown> => {
  ...
  if (componentId === "button") {
    ...
    defaults = large ? { label: "사이즈 : large" } : { label: "기본", name: "chk_1" };
    ...
    const large = definition.id.endsWith(".large");
    defaults = {
      label: large ? "사이즈 : large" : "사이즈 : medium",
      name: "rdo_2-1",
      value: undefined,
  ...
  } else if (componentId === "toggle-switch") {
    defaults = {
    ...
    defaults = {
      ...
      relatedGroups: [
        {
          title: "관련서비스/민원",
          links: ["영문 주민등록표등본", "영문 주민등록표초본", "주민등록표등본"].map((label) => ({
            label,
            href: "#",
          })),
        },
    ...
    defaults = {
      label: "레이블",
      text: "TTS 기능이란 화면에 표시된 주요 안내, 입력 서식 설명, 업무 진행 상태 등의 텍스트를 사용자가 요청했을 때 음성으로 읽어주는 보조적 사용자 지원 기능을 말하며, 이는 시각 정보의 대체가 아닌 보완 수단으로서 다른 접근성 기능(글자 크기 조절, 대비 향상, 쉬운모드 등)과 함께 제공되는 것을 원칙으로 한다.",
      children: "레이블",
    };
