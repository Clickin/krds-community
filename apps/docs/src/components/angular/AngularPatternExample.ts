  ...
  template: `
    @switch (patternId) {
      ...
      @case ('search') {
        <form
          role="search"
          (ngSubmit)="message = query ? '‘' + query + '’ 검색 결과 3건' : '검색어를 입력하세요.'"
        >
          <label [for]="id + '-query'">서비스 검색</label>
          <div class="pattern-inline">
            <input [id]="id + '-query'" name="query" [(ngModel)]="query" type="search" /><button
              type="submit"
            >
              검색
            </button>
      ...
      @case ('login') {
        <form [attr.aria-labelledby]="id + '-title'" (ngSubmit)="submit('로그인되었습니다.')">
      ...
      @case ('application') {
        <form
          ...
          (ngSubmit)="submit('신청 내용이 저장되었습니다.')"
        >
          ...
          ><input [id]="id + '-name'" name="name" autocomplete="name" required /><label
            [for]="id + '-purpose'"
            >신청 내용</label
          ><textarea [id]="id + '-purpose'" name="purpose" required></textarea
      ...
      @case ('policy') {
        <article [attr.aria-labelledby]="id + '-title'">
      ...
      @case ('consent') {
        <form (ngSubmit)="submit('동의 내용을 확인했습니다.')">
          <fieldset>
            <legend>약관 동의</legend>
            <details>
            ...
            <label
              ><input name="consent" [(ngModel)]="accepted" type="checkbox" required /> 이용약관에
              동의합니다.</label
      ...
      @case ('list') {
        <section [attr.aria-labelledby]="id + '-title'">
          ...
          <ul class="pattern-list">
            <li>
              <a href="#notice-1">서비스 이용 안내</a><time datetime="2026-07-01">2026.07.01</time>
            </li>
            <li>
              <a href="#notice-2">시스템 점검 안내</a><time datetime="2026-06-28">2026.06.28</time>
            </li>
          </ul>
      ...
      @case ('feedback') {
        <form (ngSubmit)="submit('의견을 보내주셔서 감사합니다.')">
          <fieldset>
            <legend>서비스 이용 의견</legend>
            <div class="pattern-inline">
              <label
                ><input [name]="id + '-satisfaction'" value="good" type="radio" required />
                만족</label
              ><label
                ><input [name]="id + '-satisfaction'" value="normal" type="radio" /> 보통</label
              ><label
                ><input [name]="id + '-satisfaction'" value="bad" type="radio" /> 불만족</label
              >
            </div>
            <label [for]="id + '-message'">의견</label><textarea [id]="id + '-message'"></textarea
      ...
      @case ('error') {
        <section role="alert" [attr.aria-labelledby]="id + '-title'">
          <h3 [id]="id + '-title'">요청을 처리하지 못했습니다.</h3>
          <p>잠시 후 다시 시도하거나 입력 내용을 확인해 주세요.</p>
          <button type="button" (click)="submit('다시 시도하는 중입니다.')">다시 시도</button
          ><a href="#help">도움말</a>
          @if (message) {
            <output role="status">{{ message }}</output>
          }
      ...
      @case ('form') {
        <form
          [attr.aria-labelledby]="id + '-title'"
          (ngSubmit)="submit('입력 내용이 저장되었습니다.')"
        >
          <h3 [id]="id + '-title'">정보 입력</h3>
          <label [for]="id + '-subject'">제목</label
          ><input [id]="id + '-subject'" name="subject" required /><label [for]="id + '-category'"
            >분류</label
          ><select [id]="id + '-category'" name="category" [(ngModel)]="sort">
      ...
      @case ('filter-sort') {
        <form
          [attr.aria-labelledby]="id + '-title'"
          (ngSubmit)="submit(sort + '으로 정렬했습니다.')"
        >
          <h3 [id]="id + '-title'">검색 결과</h3>
          <fieldset>
            <legend>필터와 정렬</legend>
            <label><input [name]="id + '-type-civil'" value="civil" type="checkbox" /> 민원</label
            ><label
              ><input [name]="id + '-type-policy'" value="policy" type="checkbox" /> 정책</label
            ><label [for]="id + '-sort'">정렬</label
            ><select [id]="id + '-sort'" name="sort" [(ngModel)]="sort">
              <option>최신순</option>
      ...
      @case ('mobile-notification') {
        <section [attr.aria-labelledby]="id + '-title'">
          ...
          <button type="button" (click)="submit('알림함을 열었습니다.')">알림함 열기</button>
          @if (message) {
            <output role="status">{{ message }}</output>
          }
        </section>
      ...
      @case ('mobile-settings') {
        <form [attr.aria-labelledby]="id + '-title'">
...
export class AngularPatternExample {
  @Input() patternId = '';
  message = '';
  query = '';
  accepted = false;
