...
{:else if patternId === 'confirm'}
  <section role="alertdialog" aria-labelledby={`${id}-title`} aria-describedby={`${id}-description`}><h3 id={`${id}-title`}>신청을 제출할까요?</h3><p id={`${id}-description`}>제출한 뒤에는 내용을 수정할 수 없습니다.</p><div class="pattern-inline"><button type="button" onclick={() => (message = '제출을 취소했습니다.')}>취소</button><button type="button" onclick={() => (message = '신청을 제출했습니다.')}>제출</button></div>{#if message}<output role="status">{message}</output>{/if}</section>
