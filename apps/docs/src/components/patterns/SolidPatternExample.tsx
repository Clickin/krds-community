...
export default function SolidPatternExample(props: PatternExampleProps) {
  const id = () => `solid-${props.patternId}`;
  const [message, setMessage] = createSignal('');
  const [query, setQuery] = createSignal('');
  const [accepted, setAccepted] = createSignal(false);
  const [push, setPush] = createSignal(true);
  const [sort, setSort] = createSignal('최신순');
  const submit = (event: Event, text: string) => {
    ...
    <>
      ...
      {props.patternId === 'application' && (
        ...
        >
          ...
          <label for={`${id()}-purpose`}>신청 내용</label>
      ...
      {props.patternId === 'personal-information' && (
        <fieldset>
      ...
      {props.patternId === 'feedback' && (
        <form onSubmit={(event) => submit(event, '의견을 보내주셔서 감사합니다.')}>
          <fieldset>
            <legend>서비스 이용 의견</legend>
            <div class="pattern-inline">
              <label>
                <input name={`${id()}-satisfaction`} value="good" type="radio" required /> 만족
              </label>
              <label>
                <input name={`${id()}-satisfaction`} value="normal" type="radio" /> 보통
              </label>
              <label>
                <input name={`${id()}-satisfaction`} value="bad" type="radio" /> 불만족
              </label>
            </div>
            <label for={`${id()}-message`}>의견</label>
      ...
      {props.patternId === 'filter-sort' && (
        ...
        >
          ...
          <fieldset>
            ...
            <label>
              <input name={`${id()}-type`} value="civil" type="checkbox" /> 민원
            </label>
            <label>
              <input name={`${id()}-type`} value="policy" type="checkbox" /> 정책
            </label>
      ...
      {props.patternId === 'mobile-notification' && (
        <section aria-labelledby={`${id()}-title`}>
      ...
      {props.patternId === 'mobile-settings' && (
      ...
      {![
        ...
        'mobile-notification',
        'mobile-settings',
      ].includes(props.patternId) && <p>알 수 없는 패턴입니다.</p>}
    </>
  );
}
