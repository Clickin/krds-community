...
실행 가능한 전체 예시는 [Storybook 포털](../storybook/index.html)에서 확인할 수 있습니다. Storybook 포털은 pages 배포 시 같은 base 아래에 복사되며, 문서 빌드만 실행한 `apps/docs/dist`에는 포함되지 않습니다.
...
구현 상태는 원본 UI·고정 fixture 대비 내부 conformance 검증을 통과한 구현만 제공합니다. 이 커뮤니티 프로젝트는 정부의 공식 구현이 아니며, 공식 KRDS 자료의 출처·라이선스 고지를 유지합니다.
