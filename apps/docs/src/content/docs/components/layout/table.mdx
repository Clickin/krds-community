...
<FrameworkPreview title="표">
  <ReactTable slot="react" caption="표" client:only="react" />
  <VueTable slot="vue" caption="표" client:only="vue" />
  <SvelteTable slot="svelte" caption="표" client:only="svelte" />
  <SolidTable slot="solid" caption="표" client:only="solid-js" />
  <AngularTable slot="angular" caption="표" client:only="@analogjs/astro-angular" />
  <AstroTable
    slot="astro"
    caption="표"
...
<PracticeExample kind="bad" title="표는 데이터 잘림 없이 표시될 수 있는 영역에 사용한다.">
  <div slot="react">
