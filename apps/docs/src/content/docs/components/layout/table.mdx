---
title: 표
description: KRDS 표 컴포넌트의 React, Vue, Svelte, SolidJS, Angular, Astro 구현 예제와 설명
category: layout
---
...
<FrameworkPreview title="표">
  <ReactTable slot="react" caption="표" client:only="react" columns={[{ key: 'name', label: '이름' }]} rows={[{ name: '홍길동' }]} />
  <VueTable slot="vue" caption="표" client:only="vue" columns={[{ key: 'name', label: '이름' }]} rows={[{ name: '홍길동' }]} />
  <SvelteTable slot="svelte" caption="표" client:only="svelte" columns={[{ key: 'name', label: '이름' }]} rows={[{ name: '홍길동' }]} />
  <SolidTable slot="solid" caption="표" client:only="solid-js" columns={[{ key: 'name', label: '이름' }]} rows={[{ name: '홍길동' }]} />
  <AngularTable slot="angular" caption="표" client:only="@analogjs/astro-angular" columns={[{ key: 'name', label: '이름' }]} rows={[{ name: '홍길동' }]} />
  <AstroTable
    slot="astro"
    caption="표"
...
// template
<krds-table caption="표"></krds-table>
```
...
<FrameworkPreview kind="bad" title="표는 데이터 잘림 없이 표시될 수 있는 영역에 사용한다.">
  <div slot="react">
    <div style="width: 320px; overflow-x: auto;">
      <ReactTable
        caption="좁은 영역에 배치된 표"
        columns={[
          { key: "name", label: "서비스명" },
          { key: "type", label: "구분" },
          { key: "status", label: "상태" },
          { key: "manager", label: "담당부서" },
          { key: "date", label: "개시일" },
        ]}
        rows={[
          {
...
<FrameworkPreview
  kind="good"
  title="화면 너비가 충분하지 않은 경우 데이터의 배열 방식의 변경을 고려한다."
>
  <div slot="react">
    <ReactTable
      caption="좁은 화면에서 배열을 변경한 표"
      columns={[
        { key: "name", label: "항목" },
        { key: "value", label: "내용" },
      ]}
      rows={[
        { name: "서비스명", value: "정부24 통합 인증 서비스" },
        { name: "구분", value: "인증" },
        { name: "상태", value: "운영 중" },
        { name: "담당부서", value: "디지털정부서비스과" },
      ]}
      client:only="react"
    />
  </div>
...
>
  <div slot="react">
    <div style="width: 320px; overflow-x: auto;">
      <ReactTable
        caption="배열을 변경하지 않고 좁은 영역에 배치한 표"
        columns={[
          { key: "name", label: "서비스명" },
          { key: "type", label: "구분" },
          { key: "status", label: "상태" },
...
<FrameworkPreview
  kind="good"
  title="화면 너비가 충분하지 않은 경우 표 영역에 가로 스크롤을 사용하는 방안을 고려한다."
>
  <div slot="react">
    <div style="overflow-x: auto;">
      <ReactTable
        caption="가로 스크롤을 지원하는 복합 표"
        columns={[
          { key: "name", label: "서비스명" },
          { key: "type", label: "구분" },
          { key: "status", label: "상태" },
