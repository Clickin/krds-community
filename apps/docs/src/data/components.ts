...
const sourceHashes: Record<ComponentId, string> = {
...
const sourceFiles: Record<ComponentId, string> = {
...
const headerComment = (framework: ComponentFramework, component: ComponentId, body: string) => {
  const lines = [
...
const snippet = (framework: ComponentFramework, component: ComponentId, body: string) =>
  headerComment(framework, component, body);

const packageNames: Record<ComponentFramework, string> = {
...
const sourcePaths: Record<ComponentFramework, string> = {
...
const frameworkSnippets = {
  button: {
    react: snippet(
      'react',
      'button',
      String.raw`import { Button } from '@krds-community/react';
...
export function SaveButton() {
  return <Button type="submit" variant="primary">저장</Button>;
    ...
    vue: snippet(
      'vue',
      'button',
      String.raw`<script setup lang="ts">
...
<Button type="submit" variant="primary">저장</Button>`,
    ...
    solid: snippet(
      'solid',
      'button',
      String.raw`import { Button } from '@krds-community/solid';
...
export function SaveButton() {
  return <Button type="submit" variant="primary">저장</Button>;
    ...
    angular: snippet(
      'angular',
      'button',
      String.raw`import { KrdsButtonComponent } from '@krds-community/angular';
...
<Button type="submit" variant="primary">저장</Button>`,
  ...
  'text-input': {
    react: snippet(
      'react',
      'text-input',
      String.raw`import { TextInput } from '@krds-community/react';
...
<TextInput bind:value={name} name="name" label="이름" hint="실명을 입력하세요." required />`,
    ...
    solid: snippet(
      'solid',
      'text-input',
      String.raw`import { TextInput } from '@krds-community/solid';
...
export function NameField() {
  return <TextInput name="name" label="이름" hint="실명을 입력하세요." required />;
    ...
    angular: snippet(
      'angular',
      'text-input',
      String.raw`import { KrdsTextInputComponent } from '@krds-community/angular';
...
<TextInput name="name" label="이름" hint="실명을 입력하세요." required />`,
  ...
  checkbox: {
    react: snippet(
      'react',
      'checkbox',
      String.raw`import { Checkbox } from '@krds-community/react';
...
<Checkbox bind:checked={accepted} name="terms" label="약관에 동의합니다." description="필수 동의" />`,
    ...
    solid: snippet(
      'solid',
      'checkbox',
      String.raw`import { Checkbox } from '@krds-community/solid';
...
export function TermsField() {
  return <Checkbox name="terms" label="약관에 동의합니다." description="필수 동의" />;
    ...
    angular: snippet(
      'angular',
      'checkbox',
      String.raw`import { KrdsCheckboxComponent } from '@krds-community/angular';
...
<Checkbox name="terms" label="약관에 동의합니다." description="필수 동의" required />`,
  ...
  radio: {
    react: snippet(
      'react',
      'radio',
      String.raw`import { Radio } from '@krds-community/react';
...
<Radio bind:checked={delivery} name="delivery" value="standard" label="일반 배송" />`,
    ...
    solid: snippet(
      'solid',
      'radio',
      String.raw`import { Radio } from '@krds-community/solid';
...
export function DeliveryChoice() {
  return <Radio name="delivery" value="standard" label="일반 배송" required />;
    ...
    angular: snippet(
      'angular',
      'radio',
      String.raw`import { KrdsRadioComponent } from '@krds-community/angular';
...
<Radio name="delivery" value="standard" label="일반 배송" required />`,
  ...
  switch: {
    react: snippet(
      'react',
      'switch',
      String.raw`import { Switch } from '@krds-community/react';
...
<Switch bind:checked={enabled} name="notifications" label="알림 받기" />`,
    ...
    solid: snippet(
      'solid',
      'switch',
      String.raw`import { Switch } from '@krds-community/solid';
...
export function NotificationsSwitch() {
  return <Switch name="notifications" label="알림 받기" />;
    ...
    angular: snippet(
      'angular',
      'switch',
      String.raw`import { KrdsSwitchComponent } from '@krds-community/angular';
...
<Switch name="notifications" label="알림 받기" checked />`,
  ...
  accordion: {
    react: snippet(
      'react',
      'accordion',
      String.raw`import { Accordion } from '@krds-community/react';
...
<Accordion {items} />`,
    ...
    solid: snippet(
      'solid',
      'accordion',
      String.raw`import { Accordion } from '@krds-community/solid';
...
export function DeliveryHelp() {
  return <Accordion items={items} />;
    ...
    angular: snippet(
      'angular',
      'accordion',
      String.raw`import { KrdsAccordionComponent, type KrdsAccordionItem } from '@krds-community/angular';
...
} satisfies Record<ComponentId, Record<ComponentFramework, string>>;
...
export const coreComponents: ComponentGuide[] = (Object.keys(baseGuidance) as ComponentId[]).map((id) => ({
  id,
  ...baseGuidance[id],
  packageNames,
  sourcePaths,
  sourceHashes: Object.fromEntries(
    (Object.keys(packageNames) as ComponentFramework[]).map((framework) => [framework, sourceHashes[id]]),
  ) as Record<ComponentFramework, string>,
  sourceSnapshot: pinnedKrdsSnapshot.ref,
  snippets: frameworkSnippets[id],
}));

export function findComponent(id: string): ComponentGuide {
  const component = coreComponents.find((item) => item.id === id);
  if (!component) throw new Error(`알 수 없는 KRDS 핵심 컴포넌트: ${id}`);
  return component;
}
