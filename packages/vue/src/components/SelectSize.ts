...
import type { KrdsOption, SelectRecipeSize } from "@krds-community/recipes";
...
export const SelectSize = defineComponent({
  name: "KrdsSelectSize",
  inheritAttrs: false,
  props: {
  ...
  setup(props, { attrs, emit, expose, slots: _slots }) {
    ...
    const generatedId = `krds-select-${useId()}-${createVueInstanceId("select-size")}`;
        ...
        props.options.map((option, optionIndex) =>
          create(
            "option",
            {
              ...
              selected: optionIndex === 0 ? "" : undefined,
            },
            option.label,
          ),
        ),
