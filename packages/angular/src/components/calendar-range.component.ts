  ...
  template: `
    <ng-template #calendarSurface>
      <div class="krds-calendar-area">
        <div class="calendar-wrap bottom" [attr.aria-label]="calendarLabel || null" tabindex="0">
...
export class KrdsCalendarRangeComponent {
  @Input() id = createStableId("krds-calendar-range");
  ...
  selectCalendarDate(cell: AngularCalendarCell): void {
    if (!cell.disabled) this.value = cell.date;
  }
  selectCalendarToday(): void {
    if (this.todayDay === null) return;
    this.value = `${this.calendarYear}.${this.padCalendarPart(this.calendarMonth)}.${this.padCalendarPart(this.todayDay)}`;
  }
  touch(): void {}
}
