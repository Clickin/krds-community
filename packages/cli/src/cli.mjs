...
const fallbackEntry = (id) => ({
  ...
  sourceFiles:
    fallbackSourceFiles[id] ?? [`upstream/krds-html/html/code/${id.replaceAll('-', '_')}.html`],
});
...
const readCanonicalSource = async (entry, repositoryRoot) => {
  ...
  for (const sourceFile of sourceFiles) {
    ...
    const source = sourcePath && (await canRead(sourcePath)) ? await readFile(sourcePath, 'utf8') : '';
...
export const renderSnippet = (metadata, body) => `${metadataHeader(metadata, metadata.framework)}\n${body.trim()}\n`;

const metadataLine = /^.*@krds-community\/(component|framework|source-version|source-hash|source):\s*(.*?)\s*(?:\*\/|-->)?$/;
...
export const createUnifiedDiff = (before, after, beforeLabel = 'target', afterLabel = 'source') => {
  ...
  return [`--- ${beforeLabel}`, `+++ ${afterLabel}`, `@@ -${oldRange} +${newRange} @@`, ...changes].join(
    '\n',
  );
};
...
const replaceComponentMetadata = (content, component) =>
  normalizeContent(content).replace(
    /(@krds-community\/component:\s*)[^\r\n*<]+/,
    `$1${component}`,
  );
...
const pasteComponent = async (parsed) => {
  ...
  if (sourcePath && useClipboard) throw new Error('Choose either --from <file> or --clipboard, not both.');
  ...
  if (result.status === 'noop') {
    console.log(`No-op: ${output} already uses source hash ${outputMetadata?.sourceHash ?? 'unknown'}.`);
