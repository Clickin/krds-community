...
describe('Astro server rendering', () => {
  it('renders recipe classes, native form semantics, and linked disclosures', async () => {
    ...
    try {
      ...
      expect(tabItems.map((item) => item.getAttribute('aria-selected'))).toEqual([
        'true',
        'false',
      ]);
      ...
      const tabPanels = Array.from(
        tabArea!.querySelectorAll<HTMLElement>('[role="tabpanel"]'),
      );
      expect(tabPanels.map((panel) => panel.className)).toEqual([
        'tab-conts active',
        'tab-conts',
      ]);
      expect(tabPanels.map((panel) => panel.hidden)).toEqual([false, true]);
      expect(
        tabPanels.map((panel) => panel.getAttribute('aria-labelledby')),
      ).toEqual(tabItems.map((item) => item.id));
  ...
  it('preserves upstream visual-context wrappers and inline text boundaries', async () => {
    ...
    try {
      ...
      expect(Array.from(spinnerForm?.children ?? []).map((element) => element.className))
        .toEqual(['form-tit', 'form-conts']);
      expect(
        spinnerForm?.querySelector(':scope > .form-tit + .form-conts > .form-spinner > #astro-spinner'),
      ).toBe(spinner);
      ...
      expect(spinnerForm?.querySelector<HTMLLabelElement>('.form-tit > label')?.htmlFor)
        .toBe(spinnerInput?.id);

      const inPageNavigation = document.querySelector<HTMLElement>(
        '#astro-in-page-navigation',
      );
      expect(inPageNavigation?.parentElement?.className).toBe(
      ...
      expect(
        Array.from(radioArea?.children ?? []).map((element) => element.className),
      ).toEqual(['krds-form-check medium', 'krds-form-check large']);
      ...
      expect(upload?.querySelector('.file-list > .total')?.textContent).toBe(
        '2개 / 10개',
      );
      expect(
        upload?.querySelector<HTMLInputElement>('input[type="file"]')?.hasAttribute(
          'countsuffix',
        ),
      ).toBe(false);
      ...
      expect(uploadRows.map((row) => Array.from(row.children).map((child) => child.className)))
        .toEqual([
          ['file-info'],
          ['file-info', 'file-hint-invalid'],
        ]);
  ...
  it('renders unique structured-list row checkbox names without invalid disabled links', async () => {
    ...
    try {
      ...
      const astroRequire = createRequire(
        resolve(process.cwd(), 'packages/astro/package.json'),
      );
      ...
      const accessibleNames = rowCheckboxes.map((checkbox) =>
    ...
    try {
      ...
      const assertSwitchRelationships = (
        ...
        const assertSwitch = (
          ...
          expect(options.every((option) => option.getAttribute('role') === 'option')).toBe(
            true,
          );
          expect(
      ...
      expect(calendarDocument.querySelector('caption')?.textContent?.trim()).toBe(
        '2002년 12월',
      );
      assertSwitchRelationships(calendarDocument, 2002, 12);
      expect(
        calendarDocument
          .querySelector('.btn-cal-switch.year')
          ?.getAttribute('aria-controls'),
      ).toBe('astro-calendar-default-year-list');
      expect(
        calendarDocument
          .querySelector('.btn-cal-switch.month')
          ?.getAttribute('aria-controls'),
      ).toBe('astro-calendar-default-month-list');
      expect(calendarDocument.querySelectorAll('.calendar-tbl td[data-date]')).toHaveLength(
        42,
      );
      expect(
        Array.from(
      ...
      expect(
        cellFor(calendarDocument, '2002.12.08')
          .querySelector('button')
          ?.getAttribute('aria-label'),
      ).toBe('8 일정있음');
      expect(
        cellFor(calendarDocument, '2002.12.30')
          .querySelector('button')
          ?.getAttribute('aria-label'),
      ).toBe('30 오늘');
      const disabledDay = cellFor(calendarDocument, '2002.12.13').querySelector(
        'button',
      );
      ...
      expect(
        rangeDocument.querySelector('.calendar-wrap')?.classList.contains('single'),
      ).toBe(false);
      expect(rangeDocument.querySelector<HTMLElement>('.calendar-wrap')?.tabIndex).toBe(0);
      expect(rangeDocument.querySelector('caption')?.textContent?.trim()).toBe(
        '2011년 02월',
      );
      ...
      expect(rangeCells.map((cell) => cell.dataset.date)).toEqual(
        Array.from(
          { length: 10 },
          (_, index) => `2011.02.${String(index + 7).padStart(2, '0')}`,
        ),
      );
      expect(
        rangeCells.every(
      ...
      expect(
        cellFor(rangeDocument, '2011.02.06')
          .querySelector('button')
          ?.getAttribute('aria-label'),
      ).toBe('6 일정있음');
      expect(
        cellFor(rangeDocument, '2011.02.20')
          .querySelector('button')
          ?.getAttribute('aria-label'),
      ).toBe('20 오늘');
      ...
      expect(
        Array.from(dateInputRoot?.children ?? []).map((element) => element.className),
      ).toEqual(['form-tit', 'form-conts', 'form-hint']);
      ...
      const formContents = dateInputRoot?.querySelector<HTMLElement>(
        ':scope > .form-conts',
      );
      ...
      expect(
        calendarContents?.querySelector(':scope > .calendar-input > input'),
      ).toBe(dateInput);
      expect(
        calendarContents?.querySelector(':scope > .krds-calendar-area'),
      ).not.toBeNull();
      ...
      expect(dateInputDocument.querySelector<HTMLElement>('.calendar-wrap')?.tabIndex).toBe(
        0,
      );
    ...
    try {
      ...
      for (const { dom, idPattern, activeIndex } of panelCases) {
        ...
        for (const taskDisclosure of taskDisclosures) {
          const button =
            taskDisclosure.querySelector<HTMLButtonElement>('.btn-conts-expand');
        ...
        expect(stepLists.map((list) => list.getAttribute('role'))).toEqual([
          'list',
          'list',
        ]);
      ...
      const disclosureRegion =
        disclosureRoot?.querySelector<HTMLElement>('.expand-wrap');
      ...
      expect(
        disclosureRegion?.querySelector('.krds-info-list')?.getAttribute('role'),
      ).toBe('list');
      expect(
        Array.from(disclosureRegion?.querySelectorAll('.krds-info-list > li') ?? []).map(
  ...
  it('renders exact modal SSR state without runtime-only attributes', async () => {
    ...
    try {
      const { default: Modal } = await server.ssrLoadModule(
        '/packages/astro/src/Modal.astro',
      );
      // Astro is package-scoped, so its container cannot be imported from the test root.
      const astroRequire = createRequire(
        resolve(process.cwd(), 'packages/astro/package.json'),
      ...
      const closedBackdrop =
        closedModal?.querySelector<HTMLElement>(':scope > .modal-back');
      ...
      const openModal =
        openDom.window.document.querySelector<HTMLElement>('#astro-modal-state');
  ...
  it('matches pinned sample and initialized header navigation SSR semantics', async () => {
    ...
    try {
      ...
      const sampleMobile = sampleMobileDocument.querySelector<HTMLElement>(
        '.krds-main-menu-mobile',
      );
      expect(sampleMobile?.className).toBe(
        'krds-main-menu-mobile sample consumer-mobile',
      );
      expect(sampleMobile?.getAttribute('style')).toBe(
        'display: block; position: static; visibility: visible;',
      );
      expect(sampleMobile?.querySelector('.gnb-utils > ul')?.className).toBe(
        'utility-list',
      );
      const sampleMobileList =
        sampleMobile?.querySelector<HTMLElement>('.menu-wrap > ul');
      const sampleMobileTabs = Array.from(
        sampleMobile?.querySelectorAll<HTMLAnchorElement>(
          '.menu-wrap .gnb-main-trigger',
        ) ?? [],
      );
      expect(sampleMobileList?.hasAttribute('role')).toBe(false);
      expect(
        Array.from(sampleMobileList?.children ?? []).map((item) =>
      ...
      expect(
        sampleMobileTabs.map((tab) => tab.getAttribute('href')),
      ).toEqual(['#mGnb-anchor1', '#mGnb-anchor2']);
      const sampleDepth3 =
        sampleMobile?.querySelector<HTMLAnchorElement>('.has-depth3');
      const sampleDepth4 =
        sampleMobile?.querySelector<HTMLAnchorElement>('.has-depth4');
      ...
      const sampleSearch =
        sampleMobile?.querySelector<HTMLInputElement>('.sch-input .krds-input');
      expect(sampleSearch?.title).toBe('Specific menu search');
      expect(sampleSearch?.hasAttribute('aria-label')).toBe(false);
      ...
      const initializedMobile =
        initializedMobileDocument.querySelector<HTMLElement>(
          '.krds-main-menu-mobile',
        );
      ...
      expect(
        initializedMobile
          ?.querySelector('.menu-wrap > ul')
          ?.getAttribute('role'),
      ).toBe('tablist');
      const initializedTabs = Array.from(
        initializedMobile?.querySelectorAll<HTMLAnchorElement>(
          '.menu-wrap .gnb-main-trigger',
        ) ?? [],
      );
      expect(initializedTabs.map((tab) => tab.id)).toEqual(['tab-0', 'tab-1']);
      expect(initializedTabs.map((tab) => tab.parentElement?.getAttribute('role')))
        .toEqual(['none', 'none']);
      expect(initializedTabs.map((tab) => tab.getAttribute('role'))).toEqual([
        'tab',
        'tab',
      ]);
      expect(initializedTabs.map((tab) => tab.getAttribute('aria-selected')))
        .toEqual(['true', 'false']);
      expect(initializedTabs.map((tab) => tab.getAttribute('aria-controls')))
        .toEqual(['mGnb-anchor1', 'mGnb-anchor2']);
      expect(initializedTabs.map((tab) => tab.classList.contains('active')))
        .toEqual([true, false]);
      const initializedPanels = Array.from(
        initializedMobile?.querySelectorAll<HTMLElement>(
          '.submenu-wrap > .gnb-sub-list',
        ) ?? [],
      );
      expect(initializedPanels.map((panel) => panel.getAttribute('role')))
        .toEqual(['tabpanel', 'tabpanel']);
      expect(initializedPanels.map((panel) => panel.getAttribute('aria-labelledby')))
        .toEqual(['tab-0', 'tab-1']);
      const initializedDepth3 =
        initializedMobile?.querySelector<HTMLAnchorElement>('.has-depth3');
      ...
      const samplePc =
        samplePcDocument.querySelector<HTMLElement>('nav.krds-main-menu');
      expect(samplePc?.className).toBe('krds-main-menu sample consumer-pc');
      expect(samplePc?.querySelector('.gnb-menu')?.hasAttribute('aria-label'))
        .toBe(false);
      const sampleMainTrigger =
        samplePc?.querySelector<HTMLButtonElement>('.gnb-main-trigger');
      const sampleMainPanel =
        sampleMainTrigger?.nextElementSibling as HTMLElement | null;
      const sampleSubTrigger =
        samplePc?.querySelector<HTMLButtonElement>('.gnb-sub-trigger');
      const sampleSubPanel =
        sampleSubTrigger?.nextElementSibling as HTMLElement | null;
      ...
      const initializedPc =
        initializedPcDocument.querySelector<HTMLElement>('nav.krds-main-menu');
      expect(initializedPc?.querySelector('.gnb-menu')?.getAttribute('aria-label'))
        .toBe('Main menu');
      ...
      expect(
        initializedMainTriggers.map((trigger) =>
          trigger.getAttribute('aria-controls'),
        ),
      ).toEqual([
        'gnb-main-menu-desktop-one',
        'gnb-main-menu-desktop-two',
      ]);
      expect(initializedMainTriggers.map((trigger) =>
      ...
      const initializedGroups = Array.from(
        initializedPc?.querySelectorAll<HTMLElement>(
          '.gnb-main-list[data-has-submenu="true"]',
        ) ?? [],
      );
      ...
      for (const group of initializedGroups) {
        ...
        expect(subTriggers.map((trigger) => trigger.classList.contains('active')))
          .toEqual([true, false]);
        expect(subTriggers.map((trigger) => trigger.getAttribute('aria-expanded')))
          .toEqual(['true', 'false']);
        expect(subTriggers.map((trigger) => trigger.getAttribute('aria-haspopup')))
          .toEqual(['true', 'true']);
      ...
      expect(
        header?.querySelector('.krds-main-menu .gnb-menu')
          ?.getAttribute('aria-label'),
      ).toBe('Main menu');
      ...
      expect(initializedOnlySpans.every((span) => span.textContent === ''))
        .toBe(true);
      ...
      expect(
        resizeOptions.map((option) =>
          option.querySelector('.sr-only')?.textContent),
      ).toEqual(['', 'Selected', '', '', '']);
      expect(header?.querySelector('[data-adjust], [data-adjust-scale]')).toBeNull();
      const allMenuButton =
        header?.querySelector<HTMLButtonElement>('.header-actions .btn-navi.all');
      ...
      const embeddedMobile =
        header?.querySelector<HTMLElement>('.krds-main-menu-mobile');
      ...
      expect(embeddedMobile?.querySelector('.gnb-utils > ul')?.className)
        .toBe('utility-list');
      const embeddedTabs = Array.from(
        embeddedMobile?.querySelectorAll<HTMLAnchorElement>(
          '.menu-wrap .gnb-main-trigger',
        ) ?? [],
      );
      expect(embeddedTabs.map((tab) => tab.getAttribute('aria-selected')))
        .toEqual(['true', 'false']);
      ...
      expect(embeddedSearch?.hasAttribute('aria-label')).toBe(false);
    } finally {
      doms.forEach((dom) => dom.window.close());
      await server.close();
    }
  ...
  it('matches pinned runtime residual markup and accessibility relationships', async () => {
    ...
    try {
      ...
      const uploadRoot =
        uploadDocument.querySelector<HTMLElement>('#astro-runtime-upload');
      const uploadInput =
      ...
      const uploadButton =
        uploadRoot?.querySelector<HTMLButtonElement>('.file-upload-btn-wrap > button');
      ...
      const currentSection =
        inPageDocument.querySelector<HTMLAnchorElement>('.in-page-navigation-list a');
      ...
      const currentLanguage =
        languageDocument.querySelector<HTMLElement>('.current-laguage > strong');
      ...
      const modal =
        modalDocument.querySelector<HTMLElement>('#astro-runtime-modal');
      const backdrop =
        modal?.querySelector<HTMLElement>(':scope > .modal-back');
      ...
      const pagination =
        paginationDocument.querySelector<HTMLElement>('.krds-pagination');
      expect(pagination?.getAttribute('role')).toBe('navigation');
      expect(pagination?.getAttribute('aria-label')).toBe('페이지 이동');
      expect(
        pagination?.querySelector('.page-navi.prev.disabled')?.getAttribute('href'),
      ).toBe('#');
      expect(
        pagination?.querySelector('.page-link.active .sr-only')?.textContent?.trim(),
      ).toBe('현재페이지');
      ...
      expect(
        selectDocuments[2].querySelector<HTMLSelectElement>('#astro-runtime-select-state')
      ...
      const inputState =
        inputStateDocument.querySelector<HTMLInputElement>('#astro-runtime-input-state');
      const inputStateGroup = inputState?.closest<HTMLElement>('.form-group');
      const inputStateMessage =
        inputStateGroup?.querySelector<HTMLElement>('.form-hint-invalid');
      ...
      expect(
        textareaDocument.querySelector('label[for="astro-runtime-textarea"]')
          ?.textContent,
      ).toBe('레이블');
      expect(
        textarea?.closest('.form-group')?.querySelector('.form-hint')?.textContent,
      ).toBe('도움말');
    } finally {
      doms.forEach((dom) => dom.window.close());
      await server.close();
