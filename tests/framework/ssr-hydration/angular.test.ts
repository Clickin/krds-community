...
const AngularHydrationFixture = Component(angularHydrationMetadata)(AngularHydrationFixtureState);
...
describe("Angular server rendering and hydration", () => {
  it("hydrates server markup in place and preserves stateful form and widget behavior", async () => {
    ...
    try {
      ...
      expect(host.querySelector('[data-testid="value-length"]')?.textContent?.trim()).toBe("14");
