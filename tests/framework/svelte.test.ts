...
describe('Svelte core component contracts', () => {
  ...
  it('preserves one collapsed space between structured-list icons and labels', async () => {
    ...
    await tick();
    expect(
      Array.from(host.querySelectorAll<HTMLButtonElement>('.card-btn button'), (button) => button.textContent),
    ).toEqual([' 공유하기', ' 찜하기']);
  ...
  it('keeps the tutorial variant on the expanded KRDS drawer shell', async () => {
    ...
    expect(drawer.querySelector('.help-panel-wrap')?.getAttribute('tabindex')).toBe('0');
    expect(drawer.querySelector<HTMLButtonElement>('#tutorial-tab')?.getAttribute('aria-selected')).toBe(
      'true',
    );
    ...
    expect(
      drawer.querySelector<HTMLAnchorElement>('#tutorial-panel .help-title a')?.getAttribute('href'),
    ).toBe('#;');
  ...
  it('renders initialized desktop and mobile navigation semantics inside the header', async () => {
    ...
    expect(
      host.querySelector('.my-drop > .drop-btn')?.getAttribute('aria-expanded'),
    ).toBe('false');
    expect(host.querySelectorAll('.item-link > .sr-only')).toHaveLength(9);

    const embeddedMobile = host.querySelector<HTMLElement>(
      'header > .krds-main-menu-mobile',
    )!;
    ...
    expect(embeddedSearch.getAttribute('aria-label')).toBeNull();
    ...
    expect(
      embeddedMobile.querySelector('.gnb-sub-trigger.has-depth3')?.getAttribute(
        'aria-expanded',
      ),
    ).toBe('false');
  });
