...
import { KrdsAdditionalComponent } from '../../packages/angular/src/additional.ts';
...
const selectHarnessMetadata = {
  standalone: true,
  imports: [ReactiveFormsModule, KrdsAdditionalComponent],
  template: `
    <form>
      <krds-select
        id="default-select"
...
describe('Angular core component contracts', () => {
  ...
  it('renders each select alias with the pinned ancestor and selected-option surface', () => {
    const fixture = TestBed.configureTestingModule({ imports: [SelectHarnessComponent] }).createComponent(
      SelectHarnessComponent,
    );
    fixture.detectChanges();
    ...
    expect(defaultGroup.querySelector<HTMLLabelElement>(':scope > .form-tit > label')?.htmlFor).toBe(
      'default-select',
    );
  ...
  it('gives structured-list-table row checkboxes unique names without duplicate label text', () => {
    const fixture = TestBed.configureTestingModule({ imports: [KrdsAdditionalComponent] }).createComponent(
  ...
  it('reproduces upstream wrapper contexts for tags, spinner, in-page navigation, and radio sizes', () => {
    ...
    const radioInputs = Array.from(radioArea.querySelectorAll<HTMLInputElement>('input[type="radio"]'));
  ...
  it('preserves one literal text separator at inline icon boundaries', () => {
    ...
    const criticalLink = fixture.nativeElement.querySelector<HTMLAnchorElement>('.critical-ban > a')!;
    ...
    expect([...tableActions, ...tableDownloads].every((button) => button.firstElementChild?.tagName === 'I')).toBe(
      true,
    );
  ...
  it('renders the pinned text-input-icon wrapper and password action', () => {
    ...
    const button = contents.querySelector<HTMLButtonElement>(':scope > button.krds-btn.medium.icon')!;
    ...
    expect(Array.from(contents.children).map((child) => `${child.tagName}:${child.className}`)).toEqual([
      'INPUT:krds-input',
      'BUTTON:krds-btn medium icon',
    ]);
  ...
  it('keeps the critical-alert list native and file-upload rows and count spacing exact', () => {
    ...
    update({
      ...
      files: [
        ...
        { id: 'deletable', name: commonName, status: 'deletable', deleteLabel: '삭제' },
        {
          ...
          status: 'error',
          deleteLabel: '삭제',
          errors: ['등록 가능한 파일 용량을 초과하였습니다.', '20MB 미만의 파일만 등록할 수 있습니다.'],
        },
        {
          id: 'downloadable',
          name: commonName,
    ...
    const uploadInput =
      fixture.nativeElement.querySelector<HTMLInputElement>('.file-upload-btn-wrap > input')!;
    const uploadButton =
      fixture.nativeElement.querySelector<HTMLButtonElement>('.file-upload-btn-wrap > button')!;
    ...
    expect(rows.slice(0, 3).every((row) => row.querySelector('.file-name')?.textContent?.endsWith(' '))).toBe(
      true,
    );
  ...
  it('keeps calendar display state distinct from selected options and native disabled state', () => {
    ...
    expect(yearOptions.find((option) => option.classList.contains('active'))?.textContent?.trim()).toBe(
      '2002년',
    );
    expect(monthOptions.every((option) => option.getAttribute('role') === 'option')).toBe(true);
    expect(yearOptions.find((option) => option.textContent?.trim() === '2003년')?.getAttribute('disabled')).toBe(
      '',
    );
    expect(monthOptions.find((option) => option.textContent?.trim() === '02월')?.getAttribute('disabled')).toBe(
      '',
    );
    expect(
      root.querySelector<HTMLTableCellElement>('td.old')?.classList.contains('disabled'),
    ).toBe(false);
    ...
    expect(root.querySelector<HTMLButtonElement>('[data-date="2024.12.13"] .btn-set-date')?.disabled).toBe(
      true,
    );
    ...
    fixture.detectChanges();
    expect(root.querySelector('caption')?.textContent?.replace(/\s+/g, ' ').trim()).toBe('2024년 11월');
    expect(
      root.querySelector<HTMLButtonElement>('.calendar-mon-wrap button.active')?.textContent?.trim(),
    ).toBe('11월');
  });
  ...
  it('owns tab roles on native buttons and synchronizes panels through selection', () => {
    ...
    const items = Array.from(fixture.nativeElement.querySelectorAll<HTMLLIElement>('.krds-tab-area li'));
    const tabs = Array.from(
      fixture.nativeElement.querySelectorAll<HTMLButtonElement>('.krds-tab-area button[role="tab"]'),
    );
    const panels = Array.from(
      fixture.nativeElement.querySelectorAll<HTMLElement>('.krds-tab-area [role="tabpanel"]'),
    );
    expect(items.map((item) => item.getAttribute('role'))).toEqual(['presentation', 'presentation']);
  ...
  it('initializes header menu relationships without changing the pinned root class', () => {
    ...
    const mainPanel = header.querySelector<HTMLElement>(`#${mainTrigger.getAttribute('aria-controls')}`)!;
    const subTrigger = mainPanel.querySelector<HTMLButtonElement>('.gnb-sub-trigger')!;
    const subPanel = header.querySelector<HTMLElement>(`#${subTrigger.getAttribute('aria-controls')}`)!;
    ...
    expect(desktopNav.getAttribute('aria-label')).toBeNull();
  ...
  it('connects tooltip aliases to a stable external tooltip description', () => {
    const harness = Component({
      ...
      template: `<krds-tooltip-box id="tip" label="tooltip-box" message="상세 도움말" />`,
    })(class {});
    const fixture = TestBed.configureTestingModule({ imports: [harness] }).createComponent(harness);
    fixture.detectChanges();
