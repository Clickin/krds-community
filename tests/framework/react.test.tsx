...
describe('React core component contracts', () => {
  ...
  it('keeps select surfaces native while wiring DOM, form, ref, and state relationships', async () => {
    ...
    expect(
      Array.from(serviceGroup.children).map((child) => child.getAttribute('class')),
    ...
    expect(service.getAttribute('aria-describedby')).toBe(
      'consumer-description service-hint',
    );
    ...
    expect(sizedGroup.querySelector<HTMLLabelElement>('.form-tit > label')?.htmlFor).toBe(
      'sized',
    );
  ...
  it('recomputes Select recipe variants without stale or misplaced consumer classes', async () => {
    ...
    expect(select.getAttribute('aria-describedby')).toBe(
      'external-description recipe-select-hint',
    );
  ...
  it('keeps help panel tab roles on focusable controls', async () => {
    ...
    expect(
      host.querySelectorAll(
        '.coach-help-process .krds-info-list.decimal > li[role="listitem"]',
      ),
    ).toHaveLength(3);
  });
  it('keeps contextual ownership while matching native dropdown and menu markup', async () => {
    ...
    expect(expandedControls.every((control) => control.getAttribute('aria-expanded') === 'false')).toBe(
      true,
    );
    ...
    expect(contextualPanelId ? host.ownerDocument.getElementById(contextualPanelId) : null).not.toBeNull();
    expect(
      expandedControls
  ...
  it('matches pinned desktop and mobile header menu runtime relationships', async () => {
    ...
    const subTriggers = Array.from(
      desktop.querySelectorAll<HTMLButtonElement>('.gnb-sub-trigger'),
    );
    const subPanels = subTriggers.map(
      (trigger) => host.ownerDocument.getElementById(trigger.getAttribute('aria-controls')!)!,
    );
    ...
    expect(
      mobile.querySelector('.gnb-sub-trigger.has-depth3')?.getAttribute('aria-expanded'),
    ).toBe('false');
  ...
  it('preserves pinned collapsed spaces at inline icon and label boundaries', async () => {
    ...
    expect(
      host.querySelector('.krds-help-panel .related-service .conts-wrap:first-child a')
        ?.lastChild?.nodeValue,
    ).toBe(' 전화');
    expect(
      host.querySelector('.krds-help-panel .related-service .conts-wrap:last-child a')
    ...
    expect(host.querySelector('#standalone-resize > .drop-btn')?.textContent).toBe(
      '화면크기 ',
    );
  ...
  it('does not autofocus an initially open modal but focuses, traps, and restores later opens', async () => {
    ...
    await act(async () => {
      modalButtons.at(-1)?.dispatchEvent(
