...
describe("Vue core component contracts", () => {
  ...
  it("recomputes select recipe modifiers from reactive props without stale classes", async () => {
    ...
    expect(selectElements[0].className).toBe("krds-form-select small is-error consumer-size");
    ...
    expect(
      host.querySelector<HTMLLabelElement>('.form-tit > label[for="state-select"]')?.htmlFor,
    ).toBe("state-select");
    expect(selectElements[1].getAttribute("aria-describedby")).toBe("state-select-hint");
    ...
    expect(host.querySelector("#state-select-hint")?.textContent).toBe("Error message");

    state.value = "success";
    await nextTick();
    expect(selectElements[1].className).toBe("krds-form-select is-success consumer-state");
    expect(selectElements[0].className).toBe("krds-form-select small is-success consumer-size");
    ...
    expect(host.querySelector("#state-select-hint")?.className).toBe("form-hint-success");
    expect(host.querySelector("#state-select-hint")?.textContent).toBe("Error hint");
  ...
  it("keeps select attrs, exposed element, native events, and form ownership on the control", async () => {
        ...
        return () =>
          h("form", { id: "select-form" }, [
            ...
            h(Select, {
              ...
              "onUpdate:modelValue": (next: string | number | boolean | string[]) => {
    ...
    expect(select.getAttribute("aria-describedby")).toBe("external-help account-select-hint");
    expect(root.querySelector("#account-select-hint")?.className).toBe("form-hint");
    expect(root.querySelector("#account-select-hint")?.textContent).toBe("Choose an account");
  ...
  it("keeps additional tab content and selection reactive to parent and user updates", async () => {
    ...
    const renderedItems = Array.from(host.querySelectorAll<HTMLElement>('[role="tablist"] > li'));
  ...
  it("renders upstream visual context wrappers around the captured controls", async () => {
    ...
    expect(iconInputButton.querySelector(".sr-only")?.textContent).toBe("입력한 비밀번호 보기");
    expect(iconInputButton.querySelector(".svg-icon")?.className).toBe("svg-icon ico-pw-visible");
    expect(
      host.querySelector(".krds-in-page-navigation-type > .krds-in-page-navigation-area"),
    ).not.toBeNull();
    ...
    expect(
      host.querySelectorAll(".krds-help-panel.expand > .help-panel-wrap > .help-conts-area"),
    ).toHaveLength(2);
  });
  ...
  it("preserves one collapsed space at every assigned inline icon and label boundary", async () => {
    ...
    expect(host.querySelector(".krds-critical-alerts a.krds-btn.link")?.textContent).toBe(
      "자세히 보기 ",
    );
    expect(
    ...
    expect(host.querySelector("footer .link-go a")?.textContent).toBe("찾아오시는 길 ");

    const header = host.querySelector<HTMLElement>("header")!;
    const utilityItems = Array.from(header.querySelectorAll<HTMLElement>(".utility-list > li"));
    ...
    expect(utilityItems[2].querySelector(".drop-btn")?.firstChild?.nodeValue).toBe("화면크기 ");
    expect(utilityItems[2].querySelector(".drop-bottom button")?.textContent).toBe(" 초기화");
    ...
    expect(helpPanel.querySelector(".help-conts .link-list a")?.firstChild?.nodeValue).toBe(
      "애플리케이션 다운로드 ",
    );
    expect(
    ...
    expect(host.querySelector(".krds-structured-list-table tbody button")?.textContent).toBe(
      " 파일 받기",
    );
  });
  ...
  it("retains evidence-backed critical alert, upload, structured-list, and modal nodes", async () => {
    ...
    expect(host.querySelector(".krds-structured-list .krds-badge.bg-light-primary")).not.toBeNull();
    expect(
      Array.from(host.querySelectorAll(".krds-structured-list .card-btn button")).map(
        (button) => button.textContent,
      ),
